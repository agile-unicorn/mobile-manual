...
![Pitch Round Overview](images/pitch_en.png)
...
- For a **story card**, the depicted victory points are used.
...
- For an **incentive card**, the bonus of the card is simultaneously the target value.
...
!!! example "Example"
    The next card in the sprint backlog is a blue story card with a value of 13, meaning that 13 dice points must be reached. Players then decide in turn whether to bid (pitch) on this card or pass.
...
!!! example "Example"
    Player A has 3 dice available because he used his CTO. Player C now has 4 dice available as he played two cards with the corresponding strengths.

### 4. Play Event Cards

**At any time** now players taking part in the pitch may play additional event cards before rolling their dice . The effects take place immediately. Any number of event cards may be played at the same time, even targeting different players.
...
!!! example "Example"
    To improve his chances (since Player C has 4 dice), Player A decides to play an event card from his hand. He plays "Quick Scrum", which allows him to add another team member from his hand to the pitch.
...
### 6. Use Incentives and Determine Winner
...
Bonuses from incentive and event card effects are applied to the dice result. **The player with the highest total wins the card** provided the target value is reached. In case of a tie, dice are rolled again (while event and incentive effects remain). If the target value is not reached, the pitch card is moved to the discard pile.

!!! example "Example"
    Dice are rolled. Player A scores a total of 15 with his 5 dice. Player C rolls a 17. Player A must decide first whether to use his unused incentive "Free Massages," which provides an additional 3 points for this pitch.
...
Additionally, each player places the team cards they used face down in front of themselves. The winner also adds the newly won card to their pile. All set aside or won cards remain face down until the end of the sprint and cannot be used again until the next sprint, and not in the immediately following pitch rounds.

!!! example "Example"
    Player A receives the story card, which he places face down along with his played team cards until the sprint ends. These cards are considered used for the current sprint. This also applies to Player C's team cards even though he did not win the pitch card. All played event cards go to the discard pile.

### 8. Change Starting Player

After each pitch round, the starting player rotates clockwise, the next pitch card is moved to the center, and the next, previously face-down card in the sprint backlog is revealed. The next pitch round then follows according to the above procedure using the remaining hand cards.
